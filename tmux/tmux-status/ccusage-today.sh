...
# Locate ccusage (tmux may have a stripped PATH)
CCUSAGE_BIN=$(command -v ccusage 2>/dev/null \
  || ls /opt/homebrew/bin/ccusage 2>/dev/null \
  || ls /usr/local/bin/ccusage 2>/dev/null \
  || true)
...
# Build command
cmd=("$CCUSAGE_BIN" daily -j -O -s "$TODAY" -u "$TODAY")
...
if mkdir "$LOCK_DIR" 2>/dev/null; then
	...
	cost=$(printf '%s' "${json:-}" | jq -r '(.totals?.totalCost // .totals?.costUSD) // 0' 2>/dev/null || printf '0')
